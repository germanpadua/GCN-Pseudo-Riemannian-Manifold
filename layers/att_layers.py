...
class GeoAtt(nn.Module):
    def __init__(self, in_features, dropout):
        super(GeoAtt, self).__init__()
        self.dropout = dropout
        self.linear = nn.Linear(1, 1, bias=True)
        self.in_features = in_features

    def forward (self, x, adj,time_dim=None):
        n = x.size(0)
        # n x 1 x d
        x_left = torch.unsqueeze(x, 1)
        x_left = x_left.expand(-1, n, -1)
        ...
        x_cat = torch.cat((x_left, x_right), dim=2)
        x_inner = z[:,time_dim:].sum(dim=-1) - z[:, 0:time_dim].sum(dim=-1)

        att_adj = self.linear(x_inner).squeeze()
        att_adj = F.sigmoid(att_adj)
        att_adj = torch.mul(adj.to_dense(), att_adj)
        return att_adj
